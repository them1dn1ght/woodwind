...
if env == "prod":
  p6 = requests.post(url=job_reset_url, headers=api_headers, data=json.dumps(composer_prod_json))
  print(p6.text)
else:
  p6 = requests.post(url=job_reset_url, headers=api_headers, data=json.dumps(composer_json))
  print(p6.text)
    ...
    "tasks": [
      ...
      },
      {
        ...
        "notebook_task": {
          ...
          "base_parameters": {
            "conductor_job_id": "{{job.id}}",
            "conductor_job_run_id": "{{job.run_id}}"
          },
          "source": "WORKSPACE"
        },
        "job_cluster_key": "Job_cluster",
...
p8 = requests.post(url=job_reset_url, headers=api_headers, data=json.dumps(conductor_json))
